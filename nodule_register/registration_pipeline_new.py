...
def match_nodules(ori, tar, final_transform, spacing, dist_thres=30):
    spacing = tuple(reversed(tar.GetSpacing()))
    ori.SetOrigin((0, 0, 0))
    tar.SetOrigin((0, 0, 0))
    ori_spacing = 512 / ori.GetSize()[0]  # 对于正常的512*512的CT，应该为1
    tar_spacing = 512 / tar.GetSize()[0]
...
def pipeline(ori_image, tar_image, ori_label, tar_label, dist_thres=30):
    final_transform = get_transform(ori_image, tar_image)
    spacing = spacing = tuple(reversed(ori_image.GetSpacing()))
    reg_idx0, reg_idx, tar_paired, ori_cen, tar_cen = match_nodules(ori_label, tar_label, final_transform, spacing, dist_thres=dist_thres)
